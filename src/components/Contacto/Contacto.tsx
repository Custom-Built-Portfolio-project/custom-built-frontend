...
const Contacto: React.FC = () => {
  ...
  // };
   ...
   if (!nameError && !phoneError.error && !emailError && !messageError) {
    try {
      // Realiza una solicitud POST al servidor con los datos del formulario usando Axios
      const response = await axios.post('URL_DEL_ENDPOINT_API', formData);
      ...
      setErrors({...errors, general: 'Error al enviar el formulario, problema con el servidor, intentelo de nuevo mas tarde.'})
    }
  } else {
    // Condición adicional: Muestra un mensaje de error si hay errores
    const errorMessage = 'Los datos del formulario contienen errores o faltan campos.';
